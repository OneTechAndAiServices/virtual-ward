...
import {
  Box, Button, Typography, Paper, CircularProgress, Grid, TextField, MenuItem, Chip
} from "@mui/material";
...
export default function EditQuestionPage() {
  ...
  // Fetch lookups and question
  useEffect(() => {
    async function fetchAll() {
      setLoading(true);
      try {
        const token = getToken();
        const [qData, alertRes, instRes, qnRes] = await Promise.all([
          axios.get(`${API_BASE}/Question/${id}`, { headers: { Authorization: `Bearer ${token}` } }),
        ...
        setInstructions(Array.isArray(instRes.data) ? instRes.data : instRes.data.data || []);
        setQuestionnaires(Array.isArray(qnRes.data) ? qnRes.data : qnRes.data.data || []);
      } catch {
        toast.error("Failed to fetch question/lookup data.");
      } finally {
        setLoading(false);
      }
    }
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        const token = getToken();
        // Remove any answer IDs for clean PUT (optional)
        const cleaned = {
          ...values,
        ...
        await axios.put(`${API_BASE}/Question/${id}`, cleaned, {
  ...
  return (
    <Paper sx={{ maxWidth: 900, mx: "auto", mt: 4, p: 4, borderRadius: 3, boxShadow: 4 }}>
      ...
      <Typography variant="h5" fontWeight={700} mb={3}>Edit Question</Typography>
      {loading ? (
