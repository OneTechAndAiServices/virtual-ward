...
export default function PatientVitalCharts() {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    const fetchVitals = async () => {
      ...
      try {
        ...
        if (Array.isArray(response.data)) {
          // Sort oldest -> latest, format date for X axis
  ...
  return (
    <Box p={2}>
      <Box display={"flex"} alignItems={"center"} mb={2}>
        <IconButton sx={{ mr: 1 }} href="/patient-vitals">
          <ArrowBackIcon
            sx={{
              bgcolor: seeGreen,
              color: "white",
      ...
      {data.length === 0 ? (
        <Typography p={4} textAlign="center" color="text.secondary">
          No vitals data found for this patient.
        </Typography>
      ) : (
        <Grid container spacing={3}>
          <Grid size={{xs:12,md:6}}>
